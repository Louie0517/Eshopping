...
public class ProcessingFrame extends JFrame implements ActionPanelProvider {
    ...
    public ProcessingFrame(CreateAccount user){
    ...
    private List<Orders> retrievePendingOrders(){
        if (currentUser.getRole() != null && currentUser.getRole().equalsIgnoreCase("admin")) {
            return ordersDAO.getAllOrdersByStatus("Processing");
        } else {
            return ordersDAO.getOrderByStatus(currentUser.getUserId(), "Processing");
        }
    }
    ...
    public JPanel createActionPanel(int row) {
        ...
        JButton btnProcess = new JButton("Ship");
    ...
    private void shipOrders(int row) {
        ...
        if (!status.equalsIgnoreCase("Processing")) {
            JOptionPane.showMessageDialog(
                this,
                "Only processed orders can be ship.",
                "Invalid Action",
        ...
        boolean success = ordersDAO.updateOrderStatus(orderId, "Shipped");
        Orders order = ordersDAO.getOrderById(orderId);
        if (success) {
            JOptionPane.showMessageDialog(
                this,
                "Order is now Shipping.",
                "Success",
                JOptionPane.INFORMATION_MESSAGE
            );

             if (order != null && order.getEmail() != null) {
                OnShippedOrder.sendOnShippedOrderEmail(
                    order.getEmail(),
            ...
            displayOrders("Shipped");

            if (topBar != null) {
                topBar.refreshAllBadges();
            }

        } else {
            JOptionPane.showMessageDialog(
                this,
                "Failed to ship order. It may have already been updated.",
                "Error",
                JOptionPane.ERROR_MESSAGE
            );
        }
    ...
    private void displayOrders(String status){
        ...
        if(orders.isEmpty()){
            ...
            tablePanel.setBorder(createBorder("Processing"));
    ...
    private JPanel createEmptyState() {
        ...
        JLabel titleLabel = new JLabel("No Processing Orders Found");
